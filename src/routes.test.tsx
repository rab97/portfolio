...
test('le pagine vere sono indicizzabili', () => {
  ...
  //
  // C'era un interruttore, `PLACEHOLDER_CONTENT`, che sospendeva
  // l'indicizzazione ovunque finché i testi erano segnaposto inventati. I
  // testi ora sono veri e verificabili, l'interruttore è stato tolto, e
  // questo test è tornato da solo a pretendere l'indicizzazione.
  const { helmet } = prerender(homePath('it'))
  const meta = helmet.meta.toString()

  expect(meta).not.toContain('name="robots"')

  // `canonical` c'era anche prima: dichiara qual è l'originale, cosa vera a
  // prescindere dal fatto che l'originale sia indicizzato adesso.
